...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const syncMe = useCallback(async (accessToken: string | null) => {
    ...
    try {
      ...
      setState((s) => ({
        ...s,
        error: toErrorMessage(err, "Profil bilgileri alýnamadý.."),
      }));
    }
  }, []);
    ...
    (async () => {
      try {
        ...
        if (mounted) {
          setState((s) => ({
            ...s,
            error: toErrorMessage(err, "Kayýtlý oturum okunamadý.."),
          }));
        }
      } finally {
        if (mounted) {
    ...
    async (payload: RegisterPayload) => {
      ...
      try {
        ...
        setState((s) => ({
          ...s,
          loading: false,
          error: toErrorMessage(err, "Kayýt iþlemi baþarýsýz."),
        }));
        throw err;
      }
    },
  ...
  const refresh = useCallback(async () => {
    if (!state.refreshToken) {
      setState((s) => ({
        ...s,
        error: "Kayýtlý bir refresh token bulunamadý..",
      }));
      return;
    }
